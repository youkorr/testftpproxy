...
bool FTPHTTPProxy::download_file(const std::string &remote_path, httpd_req_t *req) {
  ...
  // Plus petit buffer pour les fichiers média (512 octets au lieu de 1024)
  int buffer_size = is_media_file ? 512 : 4096;
  ...
  if (is_media_file) {
    ...
    if (wdt_result != ESP_OK) {
      ESP_LOGW(TAG, "Impossible de reconfigurer le watchdog: %d", wdt_result);
    } else {
      ESP_LOGI(TAG, "Watchdog reconfiguré avec succès pour fichier média");
    }
  }
  ...
  int consecutive_chunks = 0;
  const int yield_interval = is_media_file ? 5 : 50;  // Yield plus fréquemment pour les médias
  const int wdt_reset_interval = is_media_file ? 100 : 250;  // Réduire l'intervalle de reset WDT (ms)
  
  // Pour les très grands fichiers, informer le client qu'on traite la requête
  if (is_media_file) {
    ESP_LOGI(TAG, "Début du streaming du fichier média %s", remote_path.c_str());
  }
  
  while (true) {
    // Réinitialiser le watchdog plus souvent pour éviter les timeouts
    current_time = xTaskGetTickCount() * portTICK_PERIOD_MS;
    if (current_time - last_wdt_reset >= wdt_reset_interval) {
      if (wdt_initialized) {
        esp_task_wdt_reset();
        last_wdt_reset = current_time;
        // Log moins fréquent pour éviter de surcharger les logs
    ...
    // Réduire la taille de lecture pour des fichiers média pour éviter les blocages
    int read_size = is_media_file ? buffer_size / 2 : buffer_size;
    bytes_received = recv(data_sock, buffer, read_size, 0);
